'use strict'
/**
 * Created by kaicui on 16/11/15.2223
 * 用于对响应进行统一处理
...
const returnCodeDiction= {
    ...
    "CREATE_FAIL":["1001","数据插入失败"],
    "UPDATE_FAIL":["1002","数据修改失败"],
    "REGIST_FAIL":["2001","用户注册失败"],
...
module.exports={
     ...
     * @param data
     */
    success:function ({desc="",data=undefined,reqId=""},context) {
        if(context){
            context.body = _makeResponse(returnCodeDiction.SUCCESS,desc,data);
            context.body.success=true;
            context.body.reqId=reqId;
        }else{
